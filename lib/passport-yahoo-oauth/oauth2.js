...
OAuth2Strategy.prototype._origAuthenticate = OAuth2Strategy.prototype.authenticate;
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  if (!origOAuthFn) {
    origOAuthFn = this._oauth2.getOAuthAccessToken.bind(this._oauth2);
    this._oauth2.getOAuthAccessToken = function(code, params, callback) {
      return origOAuthFn(code, params, function(err, accessToken, refreshToken, body) {
        lastAuthBody = body;
        callback(err, accessToken, refreshToken, body);
      });
    };
  }
  return this._origAuthenticate(req, options);
}
...
Strategy.prototype.userProfile = function (accessToken, done) {
  var xoauthYahooGuid = lastAuthBody.xoauth_yahoo_guid;

  //console.log('*** CJC userProfile -->', accessToken, xoauthYahooGuid);
...
//console.log('*****CJC rq', JSON.stringify(request_options));
    try{
...
console.log("CJC oauth2 --> profile",profile);
      done(null, profile);
    }catch(e) {
      done(e);
    }
